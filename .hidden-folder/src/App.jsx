...
// Helper to sanitize Firebase paths
...
await set(ref(database, `rooms/${sanitize(roomCode)}/players/${sanitize(name)}`), { answer: "", vote: [] });
};
...
await set(ref(database, `rooms/${sanitize(roomCode)}/players/${sanitize(name)}/vote`), selectedVotes);
};
